...
$query = "UPDATE registros SET observaciones = '".$texto."' WHERE id = ".$id.";";
$mysqli->query($query);
$query2 = "SELECT registros.id, tipos.icono, tipos.tipo, dia, hora, longitud, latitud, observaciones FROM registros JOIN tipos ON registros.tipo = tipos.id WHERE dia > DATE_ADD(CURRENT_TIMESTAMP, INTERVAL -7 DAY) AND usuario = ".$usuario." ORDER BY dia DESC, hora DESC;";
